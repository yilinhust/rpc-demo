...
func (s *Server) addRoute(route string, method string, handler interface{}) {
	...
	switch handler.(type) {
	case http.Handler:
		s.routes = append(s.routes, route{r: route, cr: cr, method: method, httpHandler: handler.(http.Handler)})
	case reflect.Value:
		fv := handler.(reflect.Value)
		s.routes = append(s.routes, route{r: route, cr: cr, method: method, handler: fv})
	default:
